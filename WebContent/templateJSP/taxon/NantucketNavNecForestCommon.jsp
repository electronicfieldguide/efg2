<%@page import="java.io.File,java.util.List,
	java.util.ArrayList,java.util.Iterator,java.util.Hashtable,project.efg.server.utils.EFGDataSourceHelperInterface,project.efg.util.interfaces.EFGImportConstants,project.efg.server.utils.TemplateProducer,project.efg.server.utils.TemplatePopulator,project.efg.util.interfaces.EFGQueueObjectInterface"%>
<body class="taxonpage">
<form method="post" id="efg_form" action="<%=context%>/configTaxonPage" onsubmit="return removeConstants();"><input
	type="hidden" name="<%=groupLabel%>" value="<%=groupLabelValue%>" /> <%
...
<table class="taxontitle">
	<tr>
		<td colspan="2" class="comnametaxon"><select name="<%=name%>"
			title="Select A Field From List" style="width:100px;">
			<%
					ii = 0;
					it = table.iterator();
					while (it.hasNext()) {
	...
	<tr>
		<td colspan="2" class="scinametaxon">
		<%
...
<table class="outerimageframe">
	<tr>
		...
		<table class="imageframe">
			<tr>
				...
				<table class="images">
					<tr>
						<td class="thumb">
						<div class="thumb"><select name="<%=name%>"
							title="Select An Image Field From List" style="width:100px;">
							<%
									ii = 0;
					...
					<tr>
						...
						%>
						<td class="id_text"><input size="20" type="text"
							title="ENTER IMAGE CAPTION HERE" name="<%=name%>"
							value="<%=fieldValue%>" /></td>
						<%
									if ((zz + 1) < mediaResourceFields.size()) {
		...
		<div class="photocred"><input size="80" type="text"
			title="ENTER IMAGE CREDITS HERE" name="<%=name%>"
			value="<%=fieldValue%>" /></div>
		<%
 ...
 %>
				<p class="detail_text"><strong><input size="20"
					type="text" title="" name="<%=characterText%>"
					value="<%=characterValue%>" /></strong> <select name="<%=name%>"
 ...
 %>
		...
		String creditsgroupLabelValue = (String) groupTable
				.get(creditsgroupLabel);
		if (creditsgroupLabelValue == null) {
			creditsgroupLabelValue = "credits";
		}
		String creditscharacterLabel = tp
				.getCurrentCharacterLabel(creditsname);
		String creditscharacterLabelValue = (String) groupTable
				.get(creditscharacterLabel);
		if (creditscharacterLabelValue == null) {
			creditscharacterLabelValue = "credit";
		}
		%>
		<!-- Static links go below. There will be a single format for adding links, with an "Add Another Link" button. Only the button should show on the taxon -->
<p class="link">
...
<a class="link" href="javascript:void(0)">2nd Sample Link</a>
 ...
 %>
  <input class="addlink" name="efg_btn" id="efg_btn"
					value="Add new link"
					onclick="javascript:AddLink('CreateMoreLinks.jsp','nameValueID',this.id)"
					type="button" />
					</p>
<hr />		
		<table class="taxoncredits">
			<tr>
				<td>
				<p class="taxoncredits">
				<input type="hidden" name="<%=creditsgroupLabel%>"
					value="<%=creditsgroupLabelValue%>" /> <strong>Credits: </strong><input
					size="100" type="text" title="ENTER CREDIT INFORMATION"
					name="<%=creditsname%>" value="<%=creditsfieldValue%>" /> <input
					type="hidden" name="<%=creditscharacterLabel%>"
					value="<%=creditscharacterLabelValue%>" /></td>
			</tr>
		</table>
		</td>
	</tr>
</table>
<%@ include file="footerTaxonPage.jsp"%></form>
