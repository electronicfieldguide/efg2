...
%>	
	...
	<td class="datasourcename" colspan="2">EFG selected: <%=displayName%></td>
		           		...
		           		<td>	
							...
							%>
				 			<select name="<%=legalName%>" size="4"  multiple="multiple" onchange="evalAnySelectedValue(this);">
	                			<%if((myList.size() > 0) && !myList.contains(EFGImportConstants.EFG_ANY)){%>
 	...
 	<%  if(doSearches != null){	%>
	<input type="submit" name="submit" id="querydatasubmitID" value="Go"  onclick="javascript:postSearch('searchPageID','designformid','form_search_id')"/>
 	<%}
 	else{%>
