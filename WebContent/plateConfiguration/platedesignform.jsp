...
<%
	...
	List mediaResourceFields = 
		dsHelper.getMediaResourceFields(displayName,datasourceName);
	...
	Hashtable groupTable = tpop.populateTable(fileName.toString(), guid, 
			EFGImportConstants.SEARCHPAGE_PDF_XSL, datasourceName, whichDatabase );
	if(groupTable == null){
...
<form method="post" action="<%=context%>/efg2pdfconfig" target="_blank" name="pdfmakerform" onsubmit="return insertQueryResultsXML();">	
		...
		<tr>
			...
			<span class="configitem">Image to display:</span> 
				<%
 						...
 						if((mediaResourceFields != null) && (mediaResourceFields.size() > 0)){
 				%>
					<select name="<%=name%>"  onChange="JavaScript:checkSelected(this);" class="mustselect">
					<%
								ii=0;
			...
			<td class="formitemcaption" colspan="2">
				...
				<tr id="spanabovebuttonid">
					<td class="captionbutton" id="captionabovebuttonid">											
						...
						onClick="javascript:AddCaptionRow('/efg2/plateConfiguration/CreateMoreFieldRows.jsp',
				...
				<tr id="spanbelowbuttonid">
					<td class="captionbutton" id="captionbelowbuttonid">
						...
						<input class="addrow" type="button" name="Add new row" value="Add row" 
						onclick="javascript:AddCaptionRow('/efg2/plateConfiguration/CreateMoreFieldRows.jsp','spanbelowbuttonid','captionbelowid','numberofcaptionsbelowid')"/>
	...
	for(int i = 0; i < PDFGUIConstants.NUMBER_OF_TITLES; i++ ) {
		...
		if(fieldValue == null){
			fieldValue =PDFGUIConstants.TITLES_STR[i];
		}
		characterLabel= tp.getCurrentCharacterLabel(name);
		characterLabelValue = (String)groupTable.get(characterLabel);
		if(characterLabelValue == null){
	...
	<tr>
		<td class="formitem" colspan="2">
			...
			<%
 			...
 			%>
			<input type="text" name="<%=name%>" size="<%=dropDownDize%>" value="<%=fieldName%>" class="cleardefault" title="<%=fieldName%>"/> 
			<%
 				}	
 				name =tp.getCharacter(PDFGUIConstants.isOld,PDFGUIConstants.isOld);
 				fieldValue = (String)groupTable.get(name);
	...
	<tr>
		<td class="formitem">Frame around image:<br />
			...
			<%
 			...
 			%>	
 			<input id="imageframecolor1id" name="<%=name%>" class="color"  value="<%=fieldValue%>" type="hidden"/>	
			<input type="hidden"    name="<%=characterLabel%>" value="<%=characterLabelValue%>"/> 												
		</td>
			...
			<input type="hidden"    name="<%=characterLabel%>" value="<%=characterLabelValue%>"/> 													
	 		<span class="configitem"><!--Select a color:   -->
	 			<span>
 					...
 					%>						
		 			<input id="boundingboxcolor1id" name="<%=name%>" class="color" value="<%=fieldValue%>" type="hidden">
					<input type="hidden" name="<%=characterLabel%>" value="<%=characterLabelValue%>"/> 												
	 			</span>
	 		</span>
		</td>
	...
	<input type="submit"  name="submit" id="designformsubmitID" value="Get PDF" align="middle"/>
	<input type="button"  name="saveConfigs" id="designformconfigsubmitID" value="Save PDF Configuration" onclick="javascript:saveConfig('pdfFillFormID','Enter a unique name to save your plate configuration: ')"/>
