<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1" xmlns:xalan="http://xml.apache.org/xalan" xmlns:imageList="project.efg.server.utils.ImageDisplayList" extension-element-prefixes="imageList" exclude-result-prefixes="imageList">
		...
		<table class="taxontitle">
			<tr>
				...
				<xsl:call-template name="outputname">
					...
					<xsl:with-param name="class" select="'comnametaxon'"/>
				</xsl:call-template>
			</tr>
			<tr>
				...
				<xsl:call-template name="outputItalics">
					...
					<xsl:with-param name="taxonEntry" select="$taxonEntry"/>
					<xsl:with-param name="class" select="'scinametaxon'"/>
				</xsl:call-template>
	...
	<xsl:template name="outputImage">
		...
		<xsl:if test="$index &lt; $listsize">
			...
			<td class="thumb">
				<div class="thumb">
					<a href="{$href}">
					<xsl:call-template name="outputImageSrc">
	...
	<xsl:template match="/">
		<html>
			...
			<body class="taxonpage" onunload="closeWindow()">
