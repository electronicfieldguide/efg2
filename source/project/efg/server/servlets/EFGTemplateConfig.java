...
public abstract class EFGTemplateConfig extends HttpServlet {
	 ...
	 * @param res
	 */
	public abstract void processParams(HttpServletRequest httpservletrequest,
	...
	protected TaxonPageTemplates getTaxonPageTemplateRoot(String dsName,
			String tableName) {
		if (dsName == null) {
			return null;
		}
	...
	protected XslPage getXSLPageType(TaxonPageTemplates tps, String dsName,
		...
		synchronized (mute) {
			...
			XslPage page;
			XslPageType xslPageType = getCurrentXSLPageType(tps, dsName,
					searchType);
			page = null;
			...
			for (int j = 0; j < xslPageType.getXslPageCount(); j++) {
				...
				if (currentGUIDName.equalsIgnoreCase(guid)
						|| currentUniqueName.equalsIgnoreCase(uniqueName)) {
	...
	protected XslPageType getCurrentXSLPageType(TaxonPageTemplates tps,
			String dsName, String searchType) {
		String mute = "";
		synchronized (mute) {
	...
	protected GroupType checkGroup(GroupsType groups, String groupID,
			int groupRank) {
		GroupType gt = null;
	...
	protected String createGroup(GroupsType groups, String key, String text) {
		...
		if ("gl".equalsIgnoreCase(identifier.trim())) {
			if (text != null || !text.trim().equals(""))
				group.setLabel(text);
		} else if ("gtl".equalsIgnoreCase(identifier.trim())) {
			if (text != null || !text.trim().equals(""))
				group.setText(text);
		} else {
			...
			if ("ctl".equalsIgnoreCase(identifier.trim()))
				cv.setText(text);
			else if ("cl".equalsIgnoreCase(identifier.trim())
					|| "gtl".equalsIgnoreCase(identifier.trim())) {
				if ("cl".equalsIgnoreCase(identifier.trim()))
					cv.setLabel(text);
				else
					cv.setText(text);
			} else if ("group".equalsIgnoreCase(identifier.trim())
	...
	protected XslPage getXSLPageParams(HttpServletRequest req,
		...
		return getXSLPageType(tps, dsName, searchPage, searchType, xslName,
				uniqueName, guid, jspName, isDefault);
	}
