...
public abstract class UploadServlet extends HttpServlet {
	...
	public abstract void doCustomProcessing(HttpServletResponse response,
			File[] importedFiles);
				...
				if (!fileItem.isFormField()) {
					if (fileItem.getSize() > 0) {
						...
						if (fileName != null && !fileName.trim().equals("")) {
							...
							File file = new File(getRepository(fileItem),
									fileName);
							files.add(file);
							fileItem.write(file);
						}
			...
			if (bytesProcessed != sizeTotal) {
				...
				response.getWriter().println(
						"Uploaded: " + bytesProcessed + " out of " + sizeTotal
								...
								+ Math.round(uploadRate / 1024) + " Kbs <br/>");
				response.getWriter().println(
	...
	protected void sendCompleteResponse(HttpServletResponse response,
