/**
 * $Id: EFGContextListener.java,v 1.1.1.1 2007/08/01 19:11:23 kasiedu Exp $
 *
 ...
 */
public class EFGContextListener
    implements ServletContextListener
{
	 ...
	 */
    public static void presentError(String servletName, String message, HttpServletResponse res)
        throws IOException
    {
            ...
            do
            {
                ...
                try
                {
                    ...
                    if(binStream != null)
                    {
                        ...
                        cacheAdmin.putInCache(datasourceName.toLowerCase(), tps, templateFilesGroup);
                        lastModifiedTemplateFileTable.put(datasourceName.toLowerCase(), new Long(System.currentTimeMillis()));
                    }
                }
                catch(Exception ee)
                {
    ...
    public static Map populateCacheWithDatasources(String tableName)
    {
        ...
        try
        {
            ServletAbstractFactoryInterface servFactory = EFGSpringFactory.getServletAbstractFactoryInstance();
            ...
            if(servFactory != null)
            {
                ...
                if(listInter != null && listInter.getCount() > 0)
                {
                    ...
                    do
                    {
                        ...
                        if(datasourceName != null && !"".equals(datasourceName) && displayName != null && !"".equals(displayName))
                        {
                            servletContext.log("Adding: " + datasourceName.toLowerCase());
    ...
    private void populateCacheAdmin()
    {
        ...
        if(map != null && map.size() > 0)
            try
            {
                cacheAdmin_map.putInCache(EFGImportConstants.EFG_RDB_TABLES.toLowerCase(), map);
            }
            catch(Exception ex)
            {
                cacheAdmin_map.cancelUpdate(EFGImportConstants.EFG_RDB_TABLES.toLowerCase());
            }
        map = populateCacheWithDatasources(EFGImportConstants.EFG_GLOSSARY_TABLES);
        if(map != null && map.size() > 0)
            try
            {
                cacheAdmin_map.putInCache(EFGImportConstants.EFG_GLOSSARY_TABLES.toLowerCase(), map);
            }
            catch(Exception ex)
            {
                cacheAdmin_map.cancelUpdate(EFGImportConstants.EFG_GLOSSARY_TABLES.toLowerCase());
            }
    }
...
//	 Looks for a servlet context init parameter with a
	 ...
	 */
    private void setInitialAttribute(ServletContext context, String initParamName)
    {
    ...
    {
        String fullPath = servletContext.getRealPath("/WEB-INF/classes/properties/" + File.separator + "queryKeyWords.properties");
        try
        {
            ...
            do
            {
                ...
                if(!word.startsWith("#"))
                {
                    ...
                    if(tokens.length >= 2)
                    {
                        ...
                        if(buf.length() > 0)
                            addToKeyWordsMap(tokens[0], new String(buf.toString()));
                    }
                }
            } while(true);
        }
    ...
    public static String templateFilesGroup[] = {
        "templateFiles"
    };
