...
public class ApplyXSL extends HttpServlet {
	...
	private void transform(HttpServletRequest req, HttpServletResponse res,
			Source xsl, Source xml) throws IOException {
		if (xsl != null && xml != null) {
			...
			try {
				...
				String dsName = (String) req
						.getAttribute(EFGImportConstants.DATASOURCE_NAME);
				String alldbname = (String) req.getAttribute("ALL_TABLE_NAME");
				String displayName = (String) req
						.getAttribute(EFGImportConstants.DISPLAY_NAME);
				String xslName = (String) req
						.getAttribute(EFGImportConstants.XSL_STRING);
				String guid = (String) req
						.getAttribute(EFGImportConstants.GUID);
				String searchPage = (String) req.getAttribute("search");
				String fieldName = (String) req.getAttribute("fieldName");
				String mediaresource = (String) req
				...
				transformer
						.setParameter(EFGImportConstants.XSL_STRING, xslName);
				transformer.setParameter(EFGImportConstants.GUID, guid);
				transformer.setParameter(EFGImportConstants.DATASOURCE_NAME,
						dsName.toLowerCase());
				if (req.getAttribute("taxon_count") != null)
					transformer.setParameter("taxon_count", (String) req
							.getAttribute("taxon_count"));
				if (displayName != null && !displayName.trim().equals("")) {
					transformer.setParameter(EFGImportConstants.DISPLAY_NAME,
							displayName);
				}
				...
				String totalNumberOfPages = (String) req
						.getAttribute("totalNumberOfPages");
				if (totalNumberOfPages != null)
					transformer.setParameter("totalNumberOfPages",
							totalNumberOfPages);
				else
					transformer.setParameter("totalNumberOfPages", "1");
				String currentPageNumber = (String) req
						.getAttribute("currentPageNumber");
				if (currentPageNumber != null)
					transformer.setParameter("currentPageNumber",
							currentPageNumber);
				else
	...
	private void applySearchPage(HttpServletRequest req, HttpServletResponse res)
			throws ServletException, IOException {
		ApplyXSLInterface applyXSL = EFGSpringFactory
				.createApplyXSLInterface("searchpage");
	...
	private void applyOtherPages(HttpServletRequest req, HttpServletResponse res)
			throws ServletException, IOException {
		ApplyXSLInterface applyXSL = EFGSpringFactory
				.createApplyXSLInterface("notsearchpage");
		Source xml = null;
		Source xsl = null;
		String nextVal = null;
		String prevVal = null;
		...
		if (sessionID != null) {
			...
			sessionBean = (EFGSessionBeanInterface) session
			...
			if (sessionID == null) {
				...
				if (sessionBean != null) {
					...
					if (list != null) {
						...
						req
								.setAttribute("totalNumberOfPages", list.size()
										+ "");
	...
	private void setSessionAttributes2Request(HttpServletRequest req,
			EFGSessionBeanInterface efgSession) {
		if (efgSession != null) {
			req.setAttribute(EFGImportConstants.DATASOURCE_NAME, efgSession
					.getAttribute(EFGImportConstants.DATASOURCE_NAME));
			req.setAttribute("ALL_TABLE_NAME", efgSession
					.getAttribute("ALL_TABLE_NAME"));
			req.setAttribute(EFGImportConstants.DISPLAY_NAME, efgSession
					.getAttribute(EFGImportConstants.DISPLAY_NAME));
			req.setAttribute(EFGImportConstants.XSL_STRING, efgSession
					.getAttribute(EFGImportConstants.XSL_STRING));
			req.setAttribute(EFGImportConstants.GUID, efgSession
					.getAttribute(EFGImportConstants.GUID));
			req.setAttribute("search", efgSession.getAttribute("search"));
			req.setAttribute("fieldName", efgSession.getAttribute("fieldName"));
			req.setAttribute("mediaResourceField", efgSession
					.getAttribute("mediaResourceField"));
		}
	}
