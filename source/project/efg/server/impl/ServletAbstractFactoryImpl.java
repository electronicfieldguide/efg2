...
public class ServletAbstractFactoryImpl extends ServletAbstractFactoryInterface {
	...
	protected EFGDataObjectListInterface createMediaResourceLists(
	 ...
	 * 
	 * @param displayName -
	 *            The display name for the current datasource
	 * @param datasourceName -
	 *            The datasource name created by database
	 */
	protected EFGMediaResourceSearchableObject createFirstField(
			String displayName, String datasourceName) {
		...
		try {
			...
			for (java.util.Iterator iter = lists.iterator(); iter.hasNext();) {
				EFGQueueObjectInterface queue = (EFGQueueObjectInterface) iter
						.next();
				if (queue.getObject(1).equalsIgnoreCase(
						EFGImportConstants.EFG_TRUE)) {// mediaresource
					if (searchObj.getMediaResourceField().equals("")) {
						searchObj.setMediaResourceField(queue.getObject(0));
					}
				}
				if (queue.getObject(2).equalsIgnoreCase(
	...
	protected EFGDataObjectListInterface createSearchableLists(
			String displayName, String datasourceName) {
	...
	protected List createAllFields(String displayName, String datasourceName) {
		...
		try {
			...
			List lists = queryExecutor.executeQueryForList(queryBuffer
					.toString(), 2);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see project.efg.servlets.efgInterface.ServletAbstractFactoryInterface#createTaxonPageFields(java.lang.String,
	 *      java.lang.String)
	 */
	protected List createTaxonPageFields(String displayName,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see project.efg.servlets.efgInterface.ServletAbstractFactoryInterface#createEFGListsFields(java.lang.String,
	 *      java.lang.String)
	 */
	protected List createEFGListsFields(String displayName,
			String datasourceName) {
		List arrList = new ArrayList();
		try {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see project.efg.servlets.efgInterface.ServletAbstractFactoryInterface#createStatisticalMeasuresFields(java.lang.String,
	 *      java.lang.String)
	 */
	protected List createStatisticalMeasuresFields(String displayName,
			String datasourceName) {
		List arrList = new ArrayList();
		try {
			...
			queryBuffer
					.append("(NUMERICValue=\"true\" or NUMERICRANGE=\"true\")");
			arrList = this.getQuery(queryBuffer.toString());
	 ...
	 * (non-Javadoc)
	 * 
	 * @see project.efg.servlets.efgInterface.ServletAbstractFactoryInterface#createCategoricalItemFields(java.lang.String,
	 *      java.lang.String)
	 */
	protected List createCategoricalItemFields(String displayName,
			String datasourceName) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see project.efg.servlets.efgInterface.ServletAbstractFactoryInterface#createNarrativeItemFields(java.lang.String,
	 *      java.lang.String)
	 */
	protected List createNarrativeItemFields(String displayName,
			String datasourceName) {
		List arrList = new ArrayList();
		try {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see project.efg.servlets.efgInterface.ServletAbstractFactoryInterface#createMediaResourceLists(java.lang.String,
	 *      java.lang.String)
	 */
	...
	private List getQuery(String query) {
		List arrList = new ArrayList();
		try {
			List lists = this.queryExecutor.executeQueryForList(query, 2);
			for (java.util.Iterator iter = lists.iterator(); iter.hasNext();) {
				EFGQueueObjectInterface queue = (EFGQueueObjectInterface) iter
						.next();
				arrList.add(queue);
	...
	private String buildAllDataSourcesQuery() {
		...
		if (this.getMainTableName() == null
				|| "".equals(this.getMainTableName().trim())) {
	...
	private EFGDisplayObjectList getLists() {
		...
		try {
			...
			List lists = this.queryExecutor.executeQueryForList(query, 3);
			for (java.util.Iterator iter = lists.iterator(); iter.hasNext();) {
				EFGQueueObjectInterface queue = (EFGQueueObjectInterface) iter
						.next();
	...
	private String getRDBQuery(String query, String displayName,
			String datasourceName) {
	...
	private String getMetadataNameFromCache(String displayName,
			String datasourceName) {
		return getMetadataName(displayName, datasourceName);
	}

	private String getMetadataName(String displayName, String datasourceName) {
