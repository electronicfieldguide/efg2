    ...
    implements EFGImportConstants
{
    ...
    public Hashtable populateTable(String fileName, String guid, String itemType, String dsName, String dbTableName)
    {
 		String mutex="";
		synchronized (mutex) {
     ...
     * @return
     */
    private Hashtable getTable(String guid, String itemType, String dsName, String dbTableName)
    {
        ...
        try
        {
            ...
            do
            {
                ...
                if(ds.equalsIgnoreCase(dsName.trim()))
                {
                    ...
                    if("XSL_FILENAME_SEARCHPAGE_PLATES".equalsIgnoreCase(xslType))
                        xslPageType = xslFileNames.getXslPlatePages();
                    else
                    if("XSL_FILENAME_SEARCHPAGE_PDF".equalsIgnoreCase(xslType))
                    ...
                    if("XSL_FILENAME_SEARCHPAGE".equalsIgnoreCase(xslType))
    ...
    private TaxonPageTemplates getTaxonPageTemplate(String dsName, String tName)
    {
        ...
        catch(NeedsRefreshException nre)
        {
            try
            {
                ts = TemplateMapObjectHandler.getTemplateFromDB(null, null, dsName, tName);
                cacheAdmin.putInCache(dsName.toLowerCase(), ts, EFGContextListener.templateFilesGroup);
            }
            catch(Exception ex)
            {
                ts = (TaxonPageTemplates)nre.getCacheContent();
