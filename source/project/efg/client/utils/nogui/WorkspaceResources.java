...
public class WorkspaceResources {
	...
	public static String removeLastSpaceFromPath(String path) {
		...
		synchronized (mutex) {
			if (path != null) {
				path = path.trim();
				int index = path
						.lastIndexOf(RegularExpresionConstants.FORWARD_SLASH);
				if (index > -1) {
	...
	public static boolean computeTemplatesHome() {
		...
		String mutex = "";
		synchronized (mutex) {
			String applicationHome = EFGImportConstants.EFGProperties
					.getProperty("efg.serverlocations.current");
			...
			if (applicationHome == null || applicationHome.trim().equals("")) {
				...
				/*
				 * JOptionPane.showConfirmDialog(null, "Application cannot find
				 * the server root because it was deleted.\n " + "Please read
				 * our documentation on how to set the server " + "
			...
			String templatesHome = EFGImportConstants.EFGProperties
					.getProperty("efg.templates.home");
			tmpFile.append(File.separator);
			...
			EFGImportConstants.EFGProperties.setProperty(
					"efg.templates.home.current", newFile);
			return true;
		}
	...
	public static boolean computeMediaResourcesHome() {
		...
		synchronized (mutex) {
			...
			if (applicationHome == null || applicationHome.trim().equals("")) {
				System.err
						.println("Application cannot find the server root because it was deleted.\n "
								+ "Please read our documentation on how to set the server "
								+ " directory\n");
				return false;
				/*
			...
			String imagesHome = EFGImportConstants.EFGProperties
					.getProperty("efg.images.home");
			...
			newFile = parseServerRoot(newFile);
			EFGImportConstants.EFGProperties.setProperty(
					"efg.mediaresources.home.current", newFile);
			return true;
		}
	...
	public static synchronized String[] getDefaultDimensions() {
		...
		if (defaultDimensions == null) {
			...
			try {
				defaultDimensions = maxDimStr
						.split(RegularExpresionConstants.COMMASEP);
			} catch (Exception ee) {
	...
	private static void writeDefaultDimesions() {
		...
		EFGImportConstants.EFGProperties.setProperty(
				"efg.thumbnails.dimensions.lists", buffer.toString());
		EFGImportConstants.EFGProperties.setProperty(
	...
	public static synchronized void doHouseKeepingbeforeClosing() {
		try {
			...
			String property = EFGImportConstants.EFGProperties
					.getProperty("efg.serverlocations.lists");
			...
			property = EFGImportConstants.EFGProperties
					.getProperty("efg.serverlocations.current");
			if (property != null && !property.trim().equals("")) {
			...
			property = EFGImportConstants.EFGProperties
					.getProperty("efg.imagemagicklocation.lists");
			 ...
			 */
			property = EFGImportConstants.EFGProperties
					.getProperty("efg.imagemagicklocation.current");
			if (property != null && !property.trim().equals("")) {
			...
			/*
			 * Write the current media resources home this depends on the
			 * current server root being set correctly
			 */
			property = EFGImportConstants.EFGProperties
					.getProperty("efg.mediaresources.home.current");
			if (property != null && !property.trim().equals("")) {
			...
			/*
			 * Write the current templates home this depends on the current
			 * server root being set correctly
			 */
			property = EFGImportConstants.EFGProperties
					.getProperty("efg.templates.home.current");
			if (property != null && !property.trim().equals("")) {
			...
			property = EFGImportConstants.EFGProperties
					.getProperty("efg.thumbnails.dimensions.lists");
			if (property == null || property.trim().equals("")) {
			...
			if (EFGImportConstants.EFGProperties
					.getProperty("efg.data.last.file") != null) {
			...
			if (EFGImportConstants.EFGProperties
					.getProperty("efg.images.last.file") != null) {
			...
			if (EFGImportConstants.EFGProperties
					.getProperty("efg.previous.zipfile.location") != null) {
			...
			if (EFGImportConstants.EFGProperties
					.getProperty("efg.efg2efg.imports.home") != null) {
	...
	private static File getWorkspaceFileName() throws Exception {
		String workspaceFileName = EFGImportConstants.EFGProperties
				.getProperty("workspace.file");
