...
public class CreateEFGUserListener implements LoginListenerInterface {
	...
	static {
		try {
			log = Logger
					.getLogger(project.efg.client.rdb.gui.CreateEFGUserListener.class);
		} catch (Exception ee) {
		}
	}
	...
	private boolean comparePasswords() {
		return new String(this.dialog.getPassword()).equals(new String(
				this.dialog.getConfirmPassword()));
	}
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if ((pwd == null) || (pwd.equals("")) || (userName == null)
				|| (userName.trim().equalsIgnoreCase("efg")) || (cpwd == null)
				|| (cpwd.equals("")) || (!this.comparePasswords())) {
			...
			if ((userName != null) && (userName.trim().equalsIgnoreCase("efg"))) {
				JOptionPane.showMessageDialog(this.dialog,
						"EFG2 does not allow the creation of a user called \""
								+ userName + "\"", "Login Error",
						JOptionPane.ERROR_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(this.dialog,
						createuserFailureMessage, "Login Error",
						JOptionPane.ERROR_MESSAGE);
			}
			this.dialog.setSuccess(false);
			return;// try again
