...
/* $Id: LoginListenerImpl.java,v 1.1.1.1 2007/08/01 19:11:17 kasiedu Exp $
* $Name:  $
...
public class LoginListenerImpl implements LoginListenerInterface {
	...
	static {
		try {
			log = Logger.getLogger(project.efg.client.rdb.gui.LoginListenerImpl.class);
		} catch (Exception ee) {
		}
	}
	...
	public LoginListenerImpl(LoginDialog dialog) {
		...
		this.myLogin =
			SpringNoGUIFactory.getLoginModule();
	...
	public void actionPerformed(ActionEvent evt) {
		...
		//factory ?
		project.efg.util.utils.DBObject dbObject = 
			new DBObject(url, m_loginName,m_password);
		
		
		if(!this.myLogin.login(dbObject)){//if a connection is successfully made to database
			if(++this.errorCounter >= LoginAbstractModule.MAX_LOGIN_ATTEMPTS){//3 attempts log out if unsuccessful
			...
			//		invoke the other
			if (!runSetUp(dbObject)) {//runset up if a connection was obtained from database
				JOptionPane.showMessageDialog(this.dialog,
						setUpError_Message,
						"Login Error", JOptionPane.ERROR_MESSAGE);
				this.dialog.setSuccess(false);
	...
	/**
	 * Do some initialiazation when a connection is obtained from the database the first time.
	 * @param dbObject
