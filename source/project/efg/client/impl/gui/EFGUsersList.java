...
import project.efg.client.utils.gui.EFGComboBox;
...
public class EFGUsersList  extends JDialog
	...
	/**
	 * A Listener for creating an EFG Power User
	 *
	 */
		...
		private void handleInput(){
			try {
				...
				if(superuserInfo != null){
					if(RunSetUp.createSuperUser(dbObject,superuserInfo)){
	...
	public class DeleteUserListener implements ActionListener {
		...
		private void deleteUser(){
			...
			if ((selectedUserName!= null) && (selectedUserName.trim().length() > 0)) {
				...
				if(EFGImportConstants.EFGProperties.getProperty("nousers").equals(selectedUserName)){
					return;
				}
