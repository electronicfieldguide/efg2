...
import project.efg.util.pdf.utils.FontObject;
...
public class PDFMaker implements PDFGUIConstants{
	...
	public int getVerticalAlignmentForImages(){
		...
		if(this.captionsBelowSet.size() > 0){
			...
			this.imageVerticalAlignment =  ((Integer)PDFConstants.alignMap.get("left")).intValue();
		}
		return this.imageVerticalAlignment;
	}
				...
				if (cv != null) {
					...
					if (label.equalsIgnoreCase("paperorientation")){
						...
						if (val != null && !val.trim().equals("")) {
							...
							Boolean bool = (Boolean)PDFConstants.orientationMap.get(val.toLowerCase());
							if(bool != null){
						...
						val = cv.getValue();
						if (val != null && !val.trim().equals("")) {
							PAPER_SIZE = (Rectangle) PDFConstants.paperSizes.get(val.trim());
						}
				} else if (label
					...
					if (label.toLowerCase().endsWith(FONT)) {
						...
						if (val != null && !val.trim().equals("")) {
							try {
								font
										.setFontName(Integer
												.parseInt((String) PDFConstants.fontFamilyMap
														.get(val.toLowerCase())));
							} catch (Exception eee) {
		...
		e.hasMoreElements();) {
			...
			if (key != null) {
				...
				if (cv != null) {
					...
					if (val != null && !val.trim().equals("")) {
						Integer al =(Integer)PDFConstants.alignMap.get(val.toLowerCase());
						if(al != null){
