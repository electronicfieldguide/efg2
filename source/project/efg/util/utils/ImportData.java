...
/**
 * Copyright Isocra Ltd 2004 You can use, modify and freely distribute this file
 * as long as you credit Isocra Ltd. There is no explicit or implied guarantee
 * of functionality associated with this file, use it at your own risk.
 * 
...
public class ImportData {
	...
	private void changeTableNamesInRDBTableToLowerCase() {
		String property = EFGImportConstants.EFGProperties.getProperty(
				"efg.rdb.tolowercase", "");
		if (property.trim().equals("")) {
			try {
				List listOfDataTableNames = EFGRDBImportUtils
						.executeQueryForList(this.jdbcTemplate, getRDBQuery(),
								2);
	...
	private void writeToFile() {
		...
		try {
			URL propsURL = this.getClass().getResource("/properties");
			...
			try {
				String filename = EFGImportConstants.EFGProperties.getProperty(
						"efg.rdb.tolowercase.file",
						"efgrdblowercase.properties");
	...
	private void processQuery(String line) {
		...
		if ("".equals(toLower)) {
			...
			this.isCreate = true;
				...
				|| (toLower.startsWith("lock")) || (toLower.startsWith("drop"))
	 ...
	 * @return
	 */
	private String replaceString(String queryN) {
