...
public class TemplateConfigProcessor {
	...
	private static GeneralCacheAdministrator cacheAdmin = EFGContextListener
			.getCacheAdmin();
	...
	private boolean writeFile(String templateName) {
		...
		if (file.exists()) {
			...
			synchronized (mute) {
				try {
					...
					LoggerUtilsServlet.logErrors(eeer);
		...
		if (done) {
			if (this.tps != null) {
				...
				cacheAdmin.putInCache(templateName.toLowerCase(), tps,
						EFGContextListener.templateFilesGroup);
				EFGContextListener.lastModifiedTemplateFileTable.put(
						templateName.toLowerCase(), new Long(file
								.lastModified()));
			}
		}
		return done;
		...
		try {
			...
			if (tps != null) {
				...
				cacheAdmin.putInCache(templateName.toLowerCase(), tps,
						EFGContextListener.templateFilesGroup);
				EFGContextListener.lastModifiedTemplateFileTable.put(
						templateName.toLowerCase(), new Long(f.lastModified()));
			}
		} catch (Exception ee) {
	...
	public String getJspPage(String guid) {
		...
		if (this.tps != null) {
			XslPageType xslPageType = this.getCurrentXSLPageType(this.dsName,
					this.searchType);
	...
	public boolean removeAConfig(String guid) {
		...
		File f = new File(file);
		if (!f.exists()) {
			LoggerUtilsServlet.logErrors(new Exception("File: " + file + " does not exists!!"));
			return false;
		}
		if (this.tps == null) {
			this.tps = this.getTaxonPageTemplateRoot();
		...
		LoggerUtilsServlet.logErrors(new Exception("Returning false"));
		return false;
	}
	...
	public List getTemplateList() {
		...
		try {
			...
			if (f.exists()) {
				...
				if (currentLastMod > lastMod.longValue()) {// if the file has
															// changed
					this.reLoad();
					EFGContextListener.lastModifiedTemplateFileTable.put(
							templateName.toLowerCase(),
							new Long(currentLastMod));
				}
			}
			if (this.tps == null) {
	...
	private TaxonPageTemplates getTaxonPageTemplateRoot() {
		...
		try {
			...
			if (templateName != null) {
				ts = (TaxonPageTemplates) cacheAdmin.getFromCache(templateName
						.toLowerCase());
			}
