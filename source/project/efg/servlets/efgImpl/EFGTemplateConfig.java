...
public abstract class EFGTemplateConfig extends HttpServlet {
	 ...
	 * @param res
	 */
	public abstract void processParams(HttpServletRequest req, 
	...
	protected String getFileLocation(String dsName) {
	...
	protected boolean marshal(FileWriter writer, TaxonPageTemplates tps) {
		try {
			String mutex = "";
			synchronized (mutex) {
				boolean done = false;
	...
	protected boolean writeFile(String dsName, TaxonPageTemplates tps) {
	...
	protected XslPageType getCurrentXSLPageType(TaxonPageTemplates tps, String dsName,
	...
	protected EFGFieldObject add2Groups(HttpServletRequest req, XslPage xslPage) {
		...
		for (Enumeration e = req.getParameterNames(); e.hasMoreElements();) {
			...
			if (key.equalsIgnoreCase(EFGImportConstants.XSL_STRING)
					...
					|| key.equalsIgnoreCase(EFGImportConstants.ALL_TABLE_NAME)
