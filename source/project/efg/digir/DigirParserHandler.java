...
public class DigirParserHandler extends DefaultHandler 
{
     ...
     * 
     * @param source the InputSource object of the xml entity
     */
     ...
     *
     * @return the string of the start attribute of the request's record element.
     */
     ...
     *
     * @return the string of the limit attribute of the request's record element.
     */
    ...
    /**
     * Returns the count string (true/false) specified in DiGIR request.
     * If the value is false, the reponse will not include the count element.
     *
    ...
    /**
     * Returns the error code if all the DataSources specified not found in
     * our database.
     *
     * @return -1 if all requested datasources cannot be found in our database, 0 otherwise
     */
    ...
    /**
     * Receive notification of the beginning of the document to take 
     * any specific actions at the beginning of a document.
     */
	...
	inStack = new Stack(); 
	/* holds the contents of the list of elements in the IN operator
	   e.g <IN>
	   <list...>
	...
	*/
	...
	_sourceType = 0; //signifies that we are processing a source element.
	_destinationType = 0; //signifies that we are processing a destination element
	_filterType = 0; //signifies that we are processing a filter element
	_inventoryType = 0; //signifies that we are processing an inventory element
	...
	inList = new EFGQueryList(); //Will hold a list of searchable type when an IN operator is seen
    }
  
    /**
     * Receive notification of the end of the document to take 
     * any specific actions at the end of a document.
     */ 
    public void endDocument() throws SAXException
    {
	//The ff conditions will have to be met to signal a datasource request error
	/*
	  1.At least one dataSource was requested     
	  2.All the DataSources was requested do not exist in our Database.
	*/
     ...
     *
     * @param locator the locator for all SAX document events
     */    
	...
	if ("IN".equalsIgnoreCase(localName)) {
		...
		if (atts != null) {
			...
			if (!dataSources.contains(destinationSource)) {
			    String message = "Error on line " +_locator.getLineNumber() + ", column " +_locator.getColumnNumber() + 
				".The DataSource: " + destinationSource + " cannot be found";			
			    errorMessages.append(message);
			}
			else {
			    //load the table for the current datasource and get the legalname
	...
	try {
		...
		str = str.replaceAll(" ",EFGImportConstants.SERVICE_LINK_FILLER);
	    ...
	    if (headerFlag && searchFlag) { //We have found the search type in the header
	...
	if ("list".equalsIgnoreCase(localName)) {
		...
		if (inStack.empty()) {
		    ...
		    inList.add(new EFGString(name + " " + "EFG:EQUALS " + value));
		}
	    }
	    stack.push(inList);
	    inStack = null;
	...
	try {
	    Class cls =  Class.forName("project.efg.digir." + localName.toUpperCase());
	    if (cls != null) {
		Class supercls = cls.getSuperclass();
		if (supercls != null) {
		    String superClassName = supercls.getName();
		    if ("project.efg.digir.COP".equalsIgnoreCase(superClassName)) {
			pop2 = stack2.pop();
			if (pop2 instanceof TempObject) {
			    ...
			    stack.push(new EFGString(name + " " + "EFG:" + localName.toUpperCase() + " " + value));
			}
			else {
			    stack2.push(pop2);	    
			}
		    }
		    else if (superClassName.equalsIgnoreCase("project.efg.digir.LOP")) {
			Object popFirst = stack.pop();
			Object popSecond = stack.pop();
			
			if ((popFirst instanceof EFGString) && (popSecond instanceof EFGString)) {
			    String obj2 = ((EFGString)popFirst).toString();
			    
			    String obj1 = ((EFGString)popSecond).toString();
			    stack.push(new EFGString(" ( " + obj1 + " " + "EFG:" + localName.toUpperCase() + " " + obj2 + " ) "));
			}
			else if ((popFirst instanceof EFGString) && (popSecond instanceof EFGQueryList)) {
			    ...
			    i++;
			    while (i < list1.size()){
				inBuff.append(" EFG:OR" + " " +(String)(list1.get(i)).toString());
				i++;
			    }
			    inBuff.append( " ) ");
			    stack.push(new EFGString(" ( " + inBuff.toString() + " " + "EFG:" + localName.toUpperCase() + " " + obj1 + " ) "));	   
			    
			    inList.clear();
			}
			else if ((popFirst instanceof EFGQueryList) && (popSecond instanceof EFGString)) {
			    ...
			    stack.push(new EFGString(" ( " + obj1 + " " + "EFG:" + localName.toUpperCase() + " " + inBuff.toString() + " ) "));	   
			    inList.clear();
			}
			else {
     ...
     *
     * @param instruction the processing instruction target
     ...
     *
     * @param e the warning information encoded as an SAXParseException
     */
    ...
    {
	//System.err.println("Warning on line " + _locator.getLineNumber() + ", column " + 
