...
import project.efg.util.interfaces.ComparableFile;
...
public class Sorter {
    ...
    public static Vector sortStringEnumeration(Enumeration enum1) {
	Vector sorted = new Vector() ;
	while ( enum1.hasMoreElements() ) 
	    orderedStringInsert((String) enum1.nextElement(), sorted) ;
	sorted.trimToSize();
	return sorted ;
    }
    ...
    static void orderedComparableInsert(ComparableFile key, Vector into) {
	int lo  = 0 ;
	int hi  = into.size() - 1 ;
	int idx = -1 ;
	ComparableFile item = null ;
	//int cmp = 0 ;

	if ( hi >= lo ) {
	    while ((hi - lo) > 1) {
		idx  = (hi-lo) / 2 + lo ;
		item = (ComparableFile) into.elementAt(idx) ;
	    ...
	    switch (hi-lo) {
	      case 0:
		item = (ComparableFile) into.elementAt(hi) ;
		idx = (item.greaterThan(key)) ? hi : hi+1;
		break ;
	      case 1:
		ComparableFile loitem = (ComparableFile) into.elementAt(lo) ;
		ComparableFile hiitem = (ComparableFile) into.elementAt(hi) ;
    ...
    public static Vector sortComparableEnumeration(Enumeration enum1) {
	Vector sorted = new Vector() ;
	while ( enum1.hasMoreElements() ) 
	    orderedComparableInsert((ComparableFile) enum1.nextElement(), sorted) ;
	sorted.trimToSize();
	return sorted ;
    }
    ...
    static void quickSortCompArray(ComparableFile array[], int lo0, int hi0) {
	int lo = lo0 ;
	int hi = hi0 ;
	ComparableFile mid = null ;
    ...
    public static ComparableFile[] sortComparableArray(ComparableFile array[],
						   boolean inplace) 
    {
	ComparableFile tosort[] =array;
	if ( ! inplace ) {
	    tosort = new ComparableFile[array.length] ;
	    System.arraycopy(array, 0, tosort, 0, array.length) ;
	}
