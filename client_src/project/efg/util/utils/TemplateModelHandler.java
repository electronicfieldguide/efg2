...
public abstract class TemplateModelHandler {
	...
	public TemplateModelHandler() {
		this.templateName = EFGImportConstants.TEMPLATE_TABLE.toLowerCase();
	 ...
	 *            Remove all keys created for this data source
	 * 
	...
	public boolean changeDisplayName(final String datasourceName,
		...
		try {
			...
			/*
			 * PreparedStatementSetter p = new PreparedStatementSetter(){ public
			 * void setValues(PreparedStatement ps) throws SQLException {
			 * ps.setString(1,displayName); ps.setString(2,
			 * datasourceName.toLowerCase()); } };
			 * this.executePreparedStatement(updateStatement.toString(), p);
			 */
			replaceKeysColumn(this.templateName.toLowerCase(), displayName,
					datasourceName);
	 ...
	 * @see project.efg.util.utils.TemplateModelHandler#add2DB(java.lang.String,
	...
	public boolean add2DB(final String key, TemplateObject templateObject) {
		...
		return insertIntoDB(key, guid, displayName, datasourceName
	 ...
	 * (non-Javadoc)
	 * 
	 * @see project.efg.util.utils.TemplateModelHandler#getTemplateConfigFromDB(project.efg.util.utils.DBObject,
	...
	public TaxonPageTemplates getTemplateConfigFromDB(DBObject dbObject,
		...
		if ("".equals(displayName) && "".equals(datasourceName)) {
			return null;
		}
	 ...
	 * @param p
	 */
	public abstract boolean executeBatchPreparedStatement(
			String updateStatement, BatchPreparedStatementSetter p);
	 ...
	 *      project.efg.util.utils.TemplateObject)
	 */
	public boolean bacthUpdateTemplateObject(String tableName,
			final String[] datasourceName, final TaxonPageTemplates[] tps) {
		if (datasourceName == null || datasourceName.length == 0 || tps == null
				|| tps.length == 0) {
			return false;
		}
		...
		try {
			...
			BatchPreparedStatementSetter p = new EFGBatchUpdatePreparedStatement(
					datasourceName, tps);
			return this.executeBatchPreparedStatement(updateStatement
					.toString(), p);
		} catch (Exception e) {
	 ...
	 *      project.efg.util.utils.TemplateObject)
	 */
	public boolean updateTemplateObject(String datasourceName,
			String tableName, TaxonPageTemplates tps) {
		...
		try {
			...
			PreparedStatementSetter p = new EFGUpdatePreparedStatement(
					datasourceName, tps);
	...
	public Hashtable getAll() {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				String key = ((EFGQueueObjectInterface) list.get(i))
						.getObject(0);
				String guid = ((EFGQueueObjectInterface) list.get(i))
						.getObject(1);
				String displayName = ((EFGQueueObjectInterface) list.get(i))
						.getObject(2);
				String dsName = ((EFGQueueObjectInterface) list.get(i))
						.getObject(3);
				String tempName = ((EFGQueueObjectInterface) list.get(i))
						.getObject(4);

				String queryString = ((EFGQueueObjectInterface) list.get(i))
						.getObject(5);
	...
	public TemplateObject getFromDB(String key) {
		...
		try {
			...
			if (list.size() > 0) {
				String guid = ((EFGQueueObjectInterface) list.get(0))
						.getObject(0);
				String displayName = ((EFGQueueObjectInterface) list.get(1))
						.getObject(0);
				String dsName = ((EFGQueueObjectInterface) list.get(2))
						.getObject(0);

				String tempName = ((EFGQueueObjectInterface) list.get(3))
						.getObject(0);
				TemplateObject tempObject = SpringFactory.getTemplateObject();
				tempObject.setGUID(guid);
				tempObject.setTemplateName(tempName);
	...
	public abstract SqlRowSet executeQueryForRowSet(String queryString)
			throws Exception;

	public abstract boolean executePreparedStatement(String updateStatement,
	 ...
	 * @return
	 */
	public boolean updateTemplateTable(String skey, TemplateObject to) {
	...
	private boolean insertIntoDB(String skey, final String guid,
			final String displayName, final String datasourceName,
